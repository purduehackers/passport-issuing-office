import { GenerationStep } from "./types/types";

export const IMAGE_GENERATION_SCALE_FACTOR = 3;

export const CURRENT_PASSPORT_VERSION = 1;

export const CROP_ASPECT = 0.8;

export const GENERATION_STEPS: readonly GenerationStep[] = [
	{
		id: "processing_portrait",
		name: "Processing portrait",
		status: "pending",
	},
	{
		id: "assigning_passport_number",
		name: "Assigning passport number",
		status: "pending",
		registerOnly: true,
	},
	{
		id: "generating_passport",
		name: "Generating passport",
		status: "pending",
	},
	{
		id: "downloading_passport",
		name: "Downloading generated passport",
		status: "pending",
	},
	{
		id: "summoning_elves",
		name: "Summoning elves",
		status: "pending",
	},
] as const;
